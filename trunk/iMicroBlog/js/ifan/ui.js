...
ifan.ui = {
	...
	setCounter: function(textarea, counter){
		...
		$E.on(textarea, 'keypress', evhandler, this, true);
		$E.on(textarea, 'keyup', evhandler, this, true);
	...
	hidePanel: function(panel){
		...
		//关闭任意panel时 自动将焦点移动到输入区.
		//try catch 防止 postarea不存在或隐藏时 此动作报错.
		try{
...
ifan.ui.ModalPanel.prototype = {
	...
	// 提取出面板显示动画效果的"总时长"参数,可实现不同的panel动画效果不同.
	animDuration : 0.6,
	...
	_toggle: function(el, isShow){
		...
		var anim = new $A(el, prop, this.animDuration , isShow ? $AE.easeIn : $AE.easeOut);
