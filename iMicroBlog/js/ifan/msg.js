ifan.msg = {
	...
	setFanfouEnv: function(site){
		this._d_url_field = 'sender_id';
		this._d_name_field = 'sender_screen_name';
		this._url_field = 'id';
		this._name_field = 'screen_name';
		this._reply_to_name_field = 'in_reply_to_user_id';
	...
	_cmp_fanfou: function(item){
		return item['in_reply_to_user_id'] == ifan.app.userinfo['id'];
	},
	...
	_handle401: function(o){
		if (o.status == 401){
			errmsg = '出错了：你的用户名/密码已变更；或者需要填写验证码，请先在网页上登录饭否，然后重新登录爱饭Plus';
	...
	_mkUserURL: function(item, img){
		return ['<a class="avatar outlink" target="_blank" href="', HOST, '/', item['user'][this._url_field], '">', img || item['user'][this._name_field], '</a>'].join('');
	},

	_mkUserURL_direct: function(item){
		return ['<a class="outlink" target="_blank" href="', HOST, '/', item[this._d_url_field], '">', item[this._d_name_field], '</a>'].join('');
	},
	...
	_genMsg: function(item){
		...
		if (item['photo_url']){
			...
			ret.push(statusURL);
			ret.push(item['id']);
			ret.push('" class="photo_status outlink"><img src="');
			ret.push(item['photo_url']);
			ret.push('" /></a>');
		}
		ret.push('<span class="msg-wrap"><span class="msg">');
		ret.push(item['text']);
		//ret.push(ifan.util.replaceFanfouPhoto(item['id'], item)); // replace photo URL (if any) with thumbnails
		ret.push('</span>');
	...
	_getAttr: function(el){
		var ret = {msgid:null, type:null, to_uid:null, to_uname:null};
		for (var k in ret){
			if (k == 'msgid'){
	...
	bindTextareaEvent: function(){
		...
		$E.on(ta, 'keydown', function(e){
			if (e.keyCode == air.Keyboard.ENTER){
				...
				this.postMsg(val);
			}else if (e.ctrlKey && e.keyCode == air.Keyboard.L){
				$E.stopEvent(e);
				ifan.app.showShortenURLPanel();
			}
		}, this, true);		
	...
	reply: function(name, in_reply_to_status_id, is_d){
		...
		this.appendText(prefix + name + ' ',true);
	},

	// 提取公共方法 : 向textarea添加文本 before是否是在前面添加
	appendText: function(text,before){
		var frm = $D.get('postform'),
			postarea = frm['status']
		var v = postarea.value;
		v = v == TEXTAREA_TIP ? '' : v;
		postarea.value =  before? text + v : v+text;
	...
	handleKeyboardNav: function(){
		var up = new YAHOO.util.KeyListener(document, {keys:air.Keyboard.UP}, {
			fn: this._navHandler(true),
			scope: this,
			correctScope: true
		});
		up.enable();
		var down = new YAHOO.util.KeyListener(document, {keys:air.Keyboard.DOWN}, {
			fn: this._navHandler(false),
			scope: this,
			correctScope: true
		});
		down.enable();
	...
	_navHandler: function(isUp){
		return function(key, e){
			$E.stopEvent(e[1]);
			var t = $E.getTarget(e[1]);
			if (t == $D.get('postform')['status']) return;
			var li = null;
			if (!this._curLi || !this.isLiVisible(this._curLi)){
				li = this.getFirstVisibleLi();
			} else {
				li = isUp ? $D.getPreviousSibling(this._curLi) : $D.getNextSibling(this._curLi);
				li.scrollIntoViewIfNeeded();
			}
			if (li) this.addSelectedClass(li);
		}
	},

	getFirstVisibleLi: function(){
		var container = $D.get('msgs'),
			lis = container.getElementsByTagName('li'),
			ct = container.scrollTop,
			ch = container.offsetHeight; 

		for (var i=0, n=lis.length; i<n; i++){
			if (this.isLiVisibleHelper(lis[i], ct, ch)) {
				return lis[i];
			} 
		}
		return null;
	...
	isLiVisible: function(li){
		if (!li) return false;
		var container = $D.get('msgs'),
			ct = container.scrollTop,
			ch = container.offsetHeight;
		
		return this.isLiVisibleHelper(li, ct, ch);
	},

	isLiVisibleHelper: function(li, scrollTop, offsetHeight){
		var t = li.offsetTop,
			h = li.offsetHeight;

		return t >= scrollTop && t <= (scrollTop + offsetHeight);
	}
}
