...
ifan.util = {
	...
	_regURL_noglobal: /(?:https?:\/\/|www\.)?(?:[\w\-]+\.[\w\-\?\.\=\&\+\%\[\]\/#;@:~!]+)/im,
	_regHttp: /^https?:\/\//,
	_fortuneData: [
		'锅头饭好吃，过头话难说。',
	...
	setSelected: function(select, val){
	...
	isURL: function(str){
		return this._regURL_noglobal.test(str);
	},

	isStartWithHTTP: function(str){
		return this._regHttp.test(str);
	}
}
